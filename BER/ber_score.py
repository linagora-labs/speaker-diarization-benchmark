...
def read_rttm_files(ref_rttm_file, hyp_rttm_file):
    ...
    for filename in hyp_rttm_dict.keys():
        assert filename in rttm_dict.keys(), f"{filename} not found in {ref_rttm_file} ({list(rttm_dict.keys())})"
...
def main():
    ...
    parser.add_argument('-d', dest='detailed_result', help='Detailed result will be saved in this file', default=None) # 'detailed_results.txt'
